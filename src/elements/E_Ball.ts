...
export default class E_Ball extends Ellipse {
  ...
  reset_() {
    ...
    this.vx =
      (Math.random() * (VX_MAX - VX_MIN) + VX_MIN) *
      (Math.random() > 0.5 ? 1 : -1) *
      scale;
    this.vy = VY * scale;
    this.ax = AX * scale;
  ...
  frameLoop_(prog: number) {
    if (
      Timing.remaining > this.confGm.ACCELERATION.TO &&
      Timing.remaining <= this.confGm.ACCELERATION.FROM
    ) {
  ...
  updateVisual_() {
    ...
    const t = Math.max(
      0,
      Math.min((speed - baseSpeed) / (maxSpeed - baseSpeed || 1), 1)
    );
