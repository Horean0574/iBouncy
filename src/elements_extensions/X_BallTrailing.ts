...
export default class X_BallTrailing {
  ...
  dotData: { birthTime: number; baseSize: number; x: number; y: number }[] = [];
  framesInterval = 5;
  ...
  frameLoop(steps: number) {
    ...
    if (this.loopIdx === 0) {
      ...
      (dot as any).w = (dot as any).h = baseSize;
      (dot as any).opacity = 0;
      (dot as any).x = Ball.cx;
      (dot as any).y = Ball.cy;
      dot.visible = true;
      this.dotData[idx] = { birthTime: performance.now() / 1000, baseSize, x: Ball.cx, y: Ball.cy };
    }
    this.updateDots_();
  }
  ...
  private updateDots_() {
    ...
    for (let i = 0; i < this.length; ++i) {
      ...
      const dot = this.dots.get(i)! as any;
