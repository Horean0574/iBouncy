...
type IntervalDuty = {
    callback: (data: IntervalDutyData) => void;
    start: number;
    step: number;
    pause: number;
    duration: number;
    paused: boolean;
    terminate: boolean;
    options: IntervalOptionsResolved;
    data: IntervalDutyData;
};

export type IntervalDutyData = {
    cnt: number;
    framesCnt: number;
    elapsed: number;
};

type IntervalOptionsResolved = {
    delay: number;
    executeTimes: number;
...
export default class EmbeddedTimer {
    #timeoutMap = new Map<symbol, TimeoutDuty>();
    #timeout2delete = new Set<symbol>();
    #intervalMap = new Map<symbol, IntervalDuty>();
    #interval2delete = new Set<symbol>();
    #FPSHandlerID: symbol | undefined;
    ...
    timeDetect(timeStamp: number): void {
        this.#timeoutFrame(timeStamp);
        this.#intervalFrame(timeStamp);
    }

    #timeoutFrame(timeStamp: number): void {
    ...
    #intervalFrame(timeStamp: number): void {
    ...
    newTimeout(callback: () => void, timeout = this.#config.minInterval): symbol {
    ...
    cancelTimeout(symbol: symbol): void {
        const duty = this.#timeoutMap.get(symbol);
    ...
    newInterval(
        ...
        options: Partial<IntervalOptionsResolved> = {},
    ): symbol {
        const resolved: IntervalOptionsResolved = Object.assign(
            {
                delay: 0,
        ...
        const toAdd: IntervalDuty = {
            callback: callback,
            start: now + resolved.delay,
            step: now + resolved.delay,
            pause: now + resolved.delay,
            duration: interval,
            paused: false,
            terminate: false,
            options: resolved,
    ...
    cancelInterval(symbol: symbol): void {
        const duty = this.#intervalMap.get(symbol);
        if (duty) duty.terminate = true;
    }

    pause(symbol: symbol): void {
    ...
    #whichType(symbol: symbol): string {
    ...
    initFPSHandler(): void {
        ...
        this.#FPSHandlerID = this.newInterval(
            (data) => {
                ...
                let front = this.#FPSTimeQueue.front();
                while (front && data.elapsed - front[0] > 1000) {
                    this.#FPSTimeQueue.pop();
                    front = this.#FPSTimeQueue.front();
                }
    ...
    destroyFPSHandler(): void {
        if (!this.#isFPSHandlerRunning()) return;
        this.cancelInterval(this.#FPSHandlerID!);
        this.#FPSHandlerID = undefined;
        this.#FPSTimeQueue.clear();
    }

    #isFPSHandlerRunning(): boolean {
    ...
    get FPS(): number {
        ...
        return (this.#cachedFPS = parseFloat(this.#calcFPS()));
    }
}
